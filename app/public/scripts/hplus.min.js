...
layer.config({
  extend: ['extend/layer.ext.css', 'skin/moon/style.css'],
  skin: 'layer-ext-moon'
  ...
  $('#side-menu').metisMenu(), $('.right-sidebar-toggle').click(function () {$('#right-sidebar').toggleClass('sidebar-open')}), $('.sidebar-container').slimScroll({
    height: '100%',
    railOpacity: .4,
    wheelStep: 10
  }), $('.open-small-chat').click(function () {$(this).children().toggleClass('fa-comments').toggleClass('fa-remove'), $('.small-chat-box').toggleClass('active')}), $('.small-chat-box .content').slimScroll({
    height: '234px',
    railOpacity: .4
